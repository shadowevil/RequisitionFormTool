...
namespace RequisitionForm
{
    ...
    {
        private const string CategoriesLocation = ".\\Data\\Purchasing\\Categories.txt";
        private const string DeliveryTypeLocation = ".\\Data\\Purchasing\\DeliveryType.txt";
        private const string ProductListLocation = ".\\Data\\Purchasing\\ProductList.txt";
        private const string XMLTemplateLocation = ".\\Data\\Purchasing\\form.xml";
        ...
        private void LoadInformation()
        {
            ...
            using (StreamReader sr = new StreamReader(File.OpenRead(ProductListLocation)))
            {
                ...
                while(!sr.EndOfStream)
                {
                    ...
                    p.VendorPartNum = lineSplitByTab[3];
        ...
        private void plantLocationListBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            lblSelectedLocation.Text = pl.LocationName + " | " + pl.PlantNum + "\n"
                + pl.Address + " " + pl.CityState + "\n"
                + pl.plantEMail + "\n"
                + pl.PlantPhoneNumber;
        }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            sfd.FileName = new string(("Purchase Dept Order Form " + DateTime.Now.ToString("MM/dd/yyyy")).Select(ch => invalidFileNameChars.Contains(ch) ? '_' : ch).ToArray());
            sfd.FileName += ".xml";
            ...
            if (sfd.ShowDialog() == DialogResult.OK)
            {
                ...
                using (StreamWriter sw = new StreamWriter(File.Create(sfd.FileName)))
                {
                    ...
                    foreach (string line in copyXMLFile)
                    {
                        ...
                        if(line.Contains("<Data ss:Type=\"String\">Bulk&#10;Plant&#10;Information"))
                        {
                            flag = new KeyValuePair<bool, string>(true, "<Data ss:Type=\"String\">Bulk&#10;Plant&#10;Information");
                            flagNum = lineNum + 1;
                        ...
                        if(line.Contains("<Data ss:Type=\"String\">Order Qty"))
                        {
                            flag = new KeyValuePair<bool, string>(true, "<Data ss:Type=\"String\">Order Qty");
                            flagNum = lineNum + 1;
                            pl = null;
                        }
                        ...
                        if(flag.Key && flagNum == lineNum)
                        {
                            ...
                            } else if(flag.Value.Contains("Bulk&#10;Plant&#10;Information"))
                            {
                                int index = line.IndexOf("</Cell>");
                                newXMLFile[lineNum] = line.Substring(0, index);
                                int leng = newXMLFile[lineNum].Length;
                                if (pl == null)
                                {
                                    MessageBox.Show("Error -1:\n\nError issue with information, not set.", "Error!", MessageBoxButtons.OK, MessageBoxIcon.Error);
                                    Environment.Exit(-1);
                                }
                                string str = pl.LocationName + " \\ " + pl.PlantNum + " \\ " + pl.Address + " \\ " +
                                    pl.CityState + " \\ " + pl.ZipCode + " \\ " + pl.plantEMail + " \\ " + pl.PlantPhoneNumber;
                                newXMLFile[lineNum] += "<Data ss:Type=\"String\">" + str + "</Data>";
                                newXMLFile[lineNum] += line.Substring(index, line.Length - index);
                                flag = new KeyValuePair<bool, string>(false, null);
                                flagNum = -1;
                                pl = null;
                            ...
                            } else if(flag.Value.Contains("Enter Special Instructions In Box Below"))
                            {
                                ...
                                flagNum = -1;
                                pl = null;
                            } else if(flag.Value.Contains("Order Qty"))
                            {
                    ...
                    {
                        if(l.Contains("<Table ss:ExpandedColumnCount=\"6\" ss:ExpandedRowCount=\""))
                        {
                            int index = l.IndexOf("13\" x:FullColumns=\"1\"");
